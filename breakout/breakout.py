...
class Breakout(Game):
    def __init__(self):
        ...
        self.ld = leaderboard.Leaderboard()
        self.score = 0
    ...
    def add_life(self):
        self.show_message('+ LIVE', centralized=True)
        self.lives += 1
    ...
    def create_menu(self):
        def on_play(button):
            ...
            self.show_message('GET READY!', 1.4,  centralized=True)
    ...
    '''def create_pause_menu(self):
        def pause_play(button):
            for b in self.menu_buttons:
                self.objects.remove(b)
            Breakout().run()

        def game_quit(button):
            self.game_over = True
            self.is_game_running = False
            self.game_over = True

        for i, (text, click_handler) in enumerate((('PLAY', pause_play), ('QUIT', game_quit))):
            b = Button(c.menu_offset_x,
                       c.menu_offset_y + (c.menu_button_h + 5) * i,
                       c.menu_button_w,
                       c.menu_button_h,
    ...
    def show_message(self, text, color=colors.WHITE, font_name='Arial', font_size=20, sleep_time=0.7,
                     centralized=False):
